...
object ConfigComponent {
  ...
  implicit val oracleConfigReader: ConfigReader[OracleConfig] = deriveReader[OracleConfig]
  implicit val httpServerConfigReader: ConfigReader[HttpServerConfig] =
    deriveReader[HttpServerConfig].map { serverConfig =>
      sys.env.get(serverConfigEnv).fold(serverConfig)(port => HttpServerConfig("localhost", port.toInt))
    }
  implicit val frontendDirectoryReader: ConfigReader[FrontendDirectoryConfig] =
    deriveReader[FrontendDirectoryConfig].map { frontendDirectoryConf =>
      sys.env.get(frontendDirectoryEnv).fold(frontendDirectoryConf)(path => FrontendDirectoryConfig(path))
    }
  implicit val configComponentReader: ConfigReader[ConfigComponent] = deriveReader[ConfigComponent]

  def mk(): Future[ConfigComponent] = {
    configSource.load[ConfigComponent] match {
      case Left(value) =>
        Future.failed(new RuntimeException(s"Config loading failed: ${value.prettyPrint()}"))
      case Right(value) =>
